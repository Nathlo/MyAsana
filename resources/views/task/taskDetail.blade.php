...
@section('title', 'My.Asana | Ma Tâche')


@section('content')
    
    <h1 class="text-center text-3xl font-bold font-bebas-neue uppercase">{{ $task->title}}</h1>
    ...
    <div class="m-10">
        ...
        <div class="col-span-11 flex flex-col pr-8 text-left sm:pl-4">
            <h3 class="text-m text-gray-600">{{ $task->project->title}}</h3>
            <p class="mb-3 pr-7 text-2xl font-semibold uppercase"> {{ $task->title }} </p>
            <p class="pr-7 text-sm">{{ $task->description }}</p>
      
            <div class="mt-5 flex flex-row text-m font-medium text-gray-500">
              <div class="">Statut :<span class="ml-2 mr-3 rounded-full bg-green-100 px-2 py-0.5 text-green-900">{{ $task->status }} </span></div>
              <div class="">Début :<span class="ml-2 mr-3 rounded-full bg-blue-100 px-2 py-0.5 text-blue-900">{{ $task->starts_at }} </span></div>
              <div class="">Fin :<span class="ml-2 mr-3 rounded-full bg-blue-100 px-2 py-0.5 text-blue-900">{{ $task->ends_at }} </span></div>
