...
@section('content')
    ...
    <form action="{{ route('project.update', $project) }}" method="POST">
        @method('PUT')
        @csrf
            <h1 class="text-center text-3xl font-bold font-bebas-neue uppercase">Modifiez ce projet</h1>
            ...
            <div class="my-4 max-w-screen-md border px-4 shadow-xl sm:mx-4 sm:rounded-xl sm:px-4 sm:py-4 md:mx-auto">
                    ...
                    <select class="rounded-md w-full" name="employee_id" id="employee">
                        @forelse ($managers as $manager)
                            <option value="{{$manager->employee->id}}" 
                                @if($manager->employee->id === $project->employee->id) 
                                    selected 
                ...
                <div class="flex flex-col gap-4 border-b py-4 sm:flex-row">
                    ...
                    <input value="{{$project->title}}" name="title" class="mb-2 w-full rounded-md border bg-white px-2 py-2 outline-none ring-blue-600 sm:mr-4 sm:mb-0 focus:ring-1" />
                    ...
                    <select class="rounded-md" name="status" id="status">
                        @forelse ($status as $state)
                            <option value="{{ $state }}" @if($state === $project->status) selected @endif>{{ $state }}</option>
                        @empty
                ...
                <div class="flex flex-col gap-4 border-b py-4 sm:flex-row">
                    ...
                    <textarea name="description" class="w-full rounded-md border bg-white px-2 py-2 outline-none ring-blue-600 focus:ring-1">{{ $project->description }}
                    </textarea>
                </div>
                <!-- Start and End Project dates-->
                <div class="flex flex-col gap-4 border-b py-4 sm:flex-row">
                    <p class="shrink-0 w-32 font-medium">Début / Fin</p>
                    <input type="date" value="{{ date('Y-m-d', strtotime($project->starts_at)) }}" name="starts_at" class="mb-2 w-full rounded-md border bg-white px-2 py-2 outline-none ring-blue-600 sm:mr-4 sm:mb-0 focus:ring-1" />
                    <input type="date" value="{{ date('Y-m-d', strtotime($project->ends_at)) }}" name="ends_at" class="w-full rounded-md border bg-white px-2 py-2 outline-none ring-blue-600 focus:ring-1" />
                </div>
